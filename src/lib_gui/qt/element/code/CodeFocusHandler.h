...
public:
	...
	struct Focus
	{
		...
		Focus() = default;
		Focus(QtCodeFile* fileIn, QtCodeArea* areaIn, 
			  QPushButton* scopeLineIn, size_t lineNumberIn, 
			  size_t columnNumberIn, Id locationIdIn, std::vector<Id> tokenIdsIn)
			: file(fileIn), area(areaIn), scopeLine(scopeLineIn), 
			  lineNumber(lineNumberIn), columnNumber(columnNumberIn), locationId(locationIdIn), tokenIds(tokenIdsIn)
		{
		}
		bool isEmpty() const
		{
